...
This process has five main sections that should be following in order:
...
The first thing to decide on are the *invariants* for your tibble subclass. Let's call ours `tbl_subclass`. By invariants, I mean requirements that must hold true for a `tbl_subclass` if it is to remain a `tbl_subclass`. These invariants are important because they form a set of consistent rules that define what should happen to a `tbl_subclass` when dplyr or vctrs operations are called on it. The two options are generally:

-   If the invariants still hold, it continues to be a `tbl_subclass`
...
It is impossible to answer this question without knowing the inner workings of a `tbl_subclass`.

If a `tbl_subclass` defines `index` as a "special" column that must be an ordered Date vector, then we've broken an invariant by replacing it with `1`. When that happens, the output is no longer a valid `tbl_subclass`, and should fall back to a `tbl_df`.

If a `tbl_subclass` has a very rigid structure where the number of rows matters, then row-binding two of them together with `vec_rbind()` *might* break that invariant. We'll need a way to check if the invariants still hold after row binding to decide if we should return a `tbl_subclass` or fall back to a `tbl_df`.
...
Once you've decided on the invariants for your class, you'll need to encode that logic into a function that allows you to check on the fly if a tibble respects the invariants or not. A good name for this function is `tbl_subclass_reconstructable(x, to)`.

`tbl_subclass_reconstructable(x, to)` should expect `x` to be a data frame that must have its invariants checked, and `to` to be a `tbl_subclass` that you would reconstruct back to. It should always return a single `TRUE` or `FALSE`. For example, rsample has an `rset` subclass representing a specific type of resampling. For 10-fold cross-validation, the tibble has 10 rows, one for each fold. The number of rows in the subclass must never change here, otherwise it isn't a valid partitioning of the data anymore, so we check the number of rows of `x` against the number of rows in `to`. `x` is normally the result of performing some kind of vctrs or dplyr operation, and `tbl_subclass_reconstructable()` gives you the chance to decide if that result is still valid or not.
...
Next up is adding a few base R methods that are required for dplyr 1.0.0 to work. As stated in `?dplyr::dplyr_extending`, dplyr selects columns with 1d `[` (i.e. `x[loc]` where `loc` is a numeric vector), and renames with `names<-`. You'll need methods for both of these, but with `tbl_subclass_reconstruct()` in hand, it should be straightforward.
...
And that's it! We let tibble do most of the heavily lifting here, and then call our reconstruct function to ensure that the result is still a valid `tbl_subclass`, otherwise we fall back to a tibble.
...
`vec_restore(x, to)` generally has the job of restoring a proxy, `x`, back to a more complex type, `to`, after some kind of transformation has been applied to `x`. Sometimes, however, it isn't possible to restore back to the original type, and instead you have to fall back to a bare tibble. Sound familiar? This is exactly what we designed `tbl_subclass_reconstruct()` to do. For dials and rsample, the `vec_restore()` methods just look like:
...
For rsample and dials, the `dplyr_reconstruct()` method is identical to the `vec_restore()` method and just uses the `tbl_subclass_reconstruct()` function we created earlier.
...
The default method of `dplyr_col_modify()` for data frames will completely destruct `data` down to a bare data frame and update the columns by iterating over the `cols` calling `data[[col_name]] <- cols[[col_name]]` as it iterates through. At the end, it calls `dplyr_reconstruct()` on the updated bare data frame to attempt to reconstruct it to your original subclass. Again, I've found that overriding this is usually not required if your `tbl_subclass_reconstruct()` method called by `dplyr_reconstruct()` already handles what happens when any "special" columns get altered or additional columns are added.
...
As an example, with the "sticky" columns mentioned earlier in this post, you might want `select()` to always return these sticky columns even if the user doesn't select them. Grouped data frames do this with a message to the user that the missing group columns are also being returned. In this case, you'd create a `select.tbl_subclass()` method that preserves the sticky columns. Remember, we still encourage `[` to ignore sticky columns so that it maintains certain invariants that we expect when we implement new dplyr verbs.
...
The only way to support dplyr pre 1.0.0 was to override individual verbs. Luckily, we can use our `tbl_subclass_reconstruct()` helper to make this easier. For example, overriding `mutate()` will probably look something like this:
...
The last thing to mention before showing how this works is that when actively developing the package with `devtools::load_all()`, even if you don't export methods with `@export`, they get picked up and registered anyways by `load_all()` if they "look" like an S3 method (i.e. they look like `<generic>.<class>()`). This can be very annoying. The way around this is to use a `_` rather than a `.` when naming the function that will be lazily registered as an S3 method. So `mutate_tbl_subclass()` rather than `mutate.tbl_subclass()`, and `dplyr_reconstruct_tbl_subclass()` rather than `dplyr_reconstruct.tbl_subclass()` and to not `@export` them at all, instead relying on lazy registration.
