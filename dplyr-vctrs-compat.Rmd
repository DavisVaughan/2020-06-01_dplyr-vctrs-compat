...
The `df_reconstruct()` and `new_bare_tibble()` functions are data frame specific helpers that currently don't live anywhere, but might eventually be moved to a package in a more generic form.
...
```{r, eval = FALSE}
`[.mytibble` <- function(x, i, j, drop = FALSE, ...) {
  out <- NextMethod()
